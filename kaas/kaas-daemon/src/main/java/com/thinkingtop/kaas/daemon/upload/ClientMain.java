...
public class ClientMain {
	...
	private int ServerPort = Integer.parseInt(KaasDaemonPropertiesReader
			.getProp("uploadServerPort"));
	private String ServerAddress = KaasDaemonPropertiesReader
			.getProp("uploadServerIP");
	...
	class SocketThread extends Thread {
		...
		public void run() {
			try {
				...
				// InetAddress loalhost =
				// InetAddress.getByName("193.168.0.103");
				...
				buf = new byte[4096];
				len = dis.read(buf);
				String tempString = new String(buf, 0, len);// feedback from the
															// server

				// System.out.println(tempString);
				if (tempString.equals("notexists")) {
					System.out
							.println("file is not exsited in the server！ cannot download");
					dos.close();
					dis.close();
					socket.close();
					return;
