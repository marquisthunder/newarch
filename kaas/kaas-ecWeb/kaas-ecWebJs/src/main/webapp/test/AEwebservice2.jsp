		...
		function callback() {
			...
			if (xmlHttp.readyState == 4) {
				if (xmlHttp.status == 200) {
					...
					var goods_ids = xmlDoc.getElementsByTagName('goods_id');
					var str = '';
					for(var i=0; i<goods_ids.length;i++){
						str += '<div onClick="sendandr(' + xmlDoc.getElementsByTagName('goods_id')[i].childNodes[0].nodeValue + ')">';
						...
						str +='商品会员价格：'+xmlDoc.getElementsByTagName('promote_price')[i].childNodes[0].nodeValue+'</div><br><br>';
					}
					if(isResult){
						...
						isResult = true;
						rules(goods_ids[0].childNodes[0].nodeValue);
					}
				} else {
